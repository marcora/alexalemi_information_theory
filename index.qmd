...
Minimize the "distance/difference" (KL divergence) between the real world and your model:

$$
\min \mathcal{S}[\text{real world};\text{model}]
$$

$$
\min \mathcal{S}[p(x);q(x)]
$$

$$
\min_q \sum_i p(x_i)\ \log\frac{p(x_i)}{q(x_i)}
$$
$$
\min_q \sum_i p(x_i)\ \log \frac{p(x_i)}{q(x_i)} = \sum p(x_i) \log p(x_i) - \min_q \sum_i p(x_i)\ \log q(x_i)
$$
which is equivalent to minimizing the **cross-entropy**:

$$
\min_q \sum_i p(x_i)\ \log q(x_i)
$$
But we do not know $p(x)$. What we know is xxx
