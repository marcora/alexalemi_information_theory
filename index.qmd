...
This conversion from odds (or dB) to probability is valid only when the odds are between two complementary hypotheses, i.e., $H_2 = \lnot H_1$, so that $\frac{P(H_1)}{P(H_2)}=\frac{P(H_1)}{P(\lnot H_1)}=\frac{P(H_1)}{1-P(H_1)}$.
