...
The **relative entropy/Kullback–Leibler divergence** from $H_1$ to $H_2$ is the **expected weight of evidence** in favor of $H_1$ vs. $H_2$, assuming $P(D\mid H_1)$ is the true data-generating distribution.

Let $P_1 = P(D\mid H_1)$ be the true data-generating distribution and $P_2 = P(D\mid H_2)$ be the other distribution.

::: callout-note
Unlike the KL divergence (which is a function of only $P_1$ and $P_2$), the weight of evidence is a function of $P_1$, $P_2$, and the observed data $D$, and it does not assume that either $P_1$ or $P_2$ is the true data-generating distribution.
:::
...
$$\mathcal{S}[p;q] = \underbrace{\sum_x p(x) \log p(x)}_{-H(p)} - \underbrace{\sum_x p(x) \log q(x)}_{-H(p,q)}$$

$$\mathcal{S}[p;q] = -H[p] - (-H[p;q]) = - H[p] + H[p;q] = H[p;q] - H[p]$$
KL divergence is simply the extra surprise/cost incurred by using distribution $q$ to approximate distribution $p$.
